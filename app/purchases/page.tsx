...
export default function PurchasesPage() {
  ...
  useEffect(() => {
    if (cart.length > 0 || selectedSupplierId || discount !== "0" || deliveryFee !== "0" || paidAmount !== "0" || applyVat || applyWht) {
      const data = { cart, selectedSupplierId, suppSearch, discount, deliveryFee, applyVat, applyWht, paidAmount, invoiceDate };
  ...
  const submitInvoice = async () => {
    ...
    if (!selectedSupplierId) return alert("يرجى تحديد المورد");
    setLoading(true);
    const res = await fetch("/api/purchases", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ personId: selectedSupplierId, items: cart, paidAmount: paid, discount: discountVal, deliveryFee: deliveryVal, applyVat, applyWht, paymentMethod, invoiceDate }),
    });
    if (res.ok) {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                ...
                {selectedSupplier.lastInvoice && (
                  <div style={{ marginTop: "8px", fontSize: "0.82rem", color: "var(--text-secondary)", borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "8px" }}>
                    ...
                    <strong style={{ color: "var(--text-primary)" }}>
                      {fmt(selectedSupplier.lastInvoice.totalAmount)} ج.م
                    </strong>
                    {" · "}
                    {new Date(selectedSupplier.lastInvoice.date).toLocaleDateString("ar-EG")}
                    {selectedSupplier.lastInvoice.invoiceNumber && (
          ...
          <div className="glass-panel">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "10px", marginBottom: "12px" }}>
              <div className="input-group">
                <label>المبلغ المدفوع للمورد</label>
                <input
